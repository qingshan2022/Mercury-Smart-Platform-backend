package com.mercury.search.job;
...
import com.mercury.search.esdao.ArticleEsDao;
import com.mercury.search.esdao.UserEsDao;
...
public class CanalStarter implements CommandLineRunner {
    ...
    @Value("${Canal.host}")
    private String canalforcookie;
    @Value("${Canal.port}")
    private int canalforport;
    ...
    public void run(String... args) throws Exception {
        ...
        CanalConnector connector = CanalConnectors.newSingleConnector(
                new InetSocketAddress(canalforcookie, canalforport),
                "example", "canal", "canal");
        int batchSize = 1000;
        int emptyCount = 0;
        try {
            connector.connect();
            connector.subscribe("mercury_blog.article,mercury_blog.user");
            connector.rollback();
