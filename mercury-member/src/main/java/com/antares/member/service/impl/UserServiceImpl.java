...
import com.mercury.member.mapper.FollowMapper;
import com.mercury.member.mapper.UserMapper;
import com.mercury.member.mapper.UserTagMapper;
import com.mercury.member.model.dto.user.*;
import com.mercury.member.model.entity.Follow;
import com.mercury.member.model.entity.User;
import com.mercury.member.model.vo.user.GiteeUser;
import com.mercury.member.model.vo.user.RecommendUserVo;
...
import com.mercury.member.utils.AlgorithmUtils;
...
import static com.mercury.common.constant.SystemConstants.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public void register(UserRegisterRequest userRegisterRequest) {
        ...
        if (!StringUtils.isEmpty(redisCode) && code.equals(redisCode.split("_")[0])) {
            ...
            throw new BusinessException(AppHttpCodeEnum.SUCCESS);
        }
    ...
    public void login(AccountLoginRequest accountLoginRequest, HttpServletResponse response) {
        ...
        //1、去数据库查询 SELECT * FROM mercury_user WHERE email = ? OR phone = ?
    ...
    private void generateTokenAndCookie(UserInfoVo userInfoVo, HttpServletResponse response){
        ...
        response.setHeader("Set-Cookie",
                cookieName + "=" + token + "; Domain=" + domain + "; Path=/; Max-Age=2592000; Secure = false; SameSite=" + sameSite);
    ...
    public void changeAvatar(HttpServletRequest request) {
        ...
        UserInfoVo userInfoVo = redisUtils.getCurrentUserWithValidation(request);
        //获取到数据库里的avatar
        String beforeAvatar = lambdaQuery().select(User::getAvatar).eq(User::getUid, userInfoVo.getUid()).one().getAvatar();
        //进行avatar匹配
        if(beforeAvatar == null){
            User user = new User();
            user.setUid(userInfoVo.getUid());
            user.setAvatar(beforeAvatar);
            updateById(user);
        } else {
            throw new BusinessException(AppHttpCodeEnum.PARAMS_ERROR);
        }
    }
